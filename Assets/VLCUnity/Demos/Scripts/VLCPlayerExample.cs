...
public class VLCPlayerExample : MonoBehaviour
{
	...
	public void Open()
	{
        ...
        // ======================= [修改这里 开始] =======================
        // 初始化 Media 载体
        Media newMedia = new Media(new Uri(trimmedPath));
        
        // 强行标注裸流格式为 H.265，否则 VLC 在解析 TCP 无头数据时会直接假死
        //newMedia.AddOption(":demux=hevc");
	...
	void CreateLibVLC()
	{
        ...
        // 将参数数组喂给构建器，同时如果你还要启用底层 Debug 日志，可将 enableDebugLogs 设为 true
		libVLC = new LibVLC(enableDebugLogs: true, vlcOptions); 
        // ======================= [修改这里 结束] =======================
