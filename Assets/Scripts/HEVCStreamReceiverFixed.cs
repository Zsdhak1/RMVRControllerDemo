...
public class HEVCStreamReceiverFixed : MonoBehaviour
{
    ...
    private void CreateTexture()
    {
        ...
        _texture = new Texture2D((int)_videoWidth, (int)_videoHeight, TextureFormat.RGBA32, false);
        _quadMaterial.mainTexture = _texture;
        
        Debug.Log($"[HEVC] 纹理创建: {_videoWidth}x{_videoHeight}");
    }

    #region LibVLC 回调（这些回调在独立线程运行，绝对不能阻塞！）
