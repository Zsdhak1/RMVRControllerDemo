...
namespace UMP
{
    public class MediaPlayerAndroid : IPlayer, IPlayerAudio, IPlayerSpu
    {
        ...
        public Uri DataSource
        {
            ...
            {
                _dataSource = value;
                string checkLocalFilePath = MediaPlayerHelper.GetDeviceRootPath() + _dataSource.ToString().Replace("file:///", "");

                if (File.Exists(checkLocalFilePath))
                    _dataSource = new Uri(checkLocalFilePath);

                if (_options.PlayerType == PlayerOptionsAndroid.PlayerTypes.Exo && MediaPlayerHelper.IsAssetsFile(_dataSource.ToString()))
                    _dataSource = new Uri(_dataSource.ToString().Replace("file:///", "asset:///"));
